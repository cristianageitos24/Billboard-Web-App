...
/**
 * Minimal header for public routes (inventory, login, signup, home guest view).
 */
export default function PublicHeader() {
  ...
  if (user) {
    return (
      <header className="flex items-center justify-between gap-4 min-h-10 px-4 py-2 text-sm border-b border-neutral-200 dark:border-neutral-800 flex-wrap">
        <Link
          href="/dashboard"
          className="font-semibold text-neutral-900 hover:text-neutral-700 shrink-0"
        >
          MarketTrace
        </Link>
        <nav className="flex items-center justify-end gap-3 flex-wrap">
          ...
          <span
            className="text-foreground/80 truncate max-w-[180px]"
  ...
  return (
    <header className="flex items-center justify-between gap-4 h-10 px-4 text-sm border-b border-neutral-200 dark:border-neutral-800">
      <Link href="/" className="font-semibold text-neutral-700 hover:text-neutral-900">
        MarketTrace
      </Link>
